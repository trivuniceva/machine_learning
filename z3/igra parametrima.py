...
def grid_search(X_train, y_train, model, param_grid, scoring='f1_micro'):
...
def randomizedSearch(X_train, y_train):
    # param_dist = {'alpha': uniform(loc=0, scale=1000)}
    # param_dist = {'fit_prior': uniform(loc=0, scale=1000)}

    # random_search_lr = RandomizedSearchCV(LogisticRegression(), param_distributions=param_dist, n_iter=10, cv=5,scoring='f1_micro')
    random_search = RandomizedSearchCV(MultinomialNB(), param_distributions=param_dist, n_iter=10, cv=5, scoring='f1_micro')

    random_search.fit(X_train, y_train)

    print("Najbolji parametri:", random_search.best_params_)
    print("Mikro F1-mera:", random_search.best_score_)

    return random_search.best_params_['force_alpha']
...
def main():
    ...
    #
    # random_search_svm = RandomizedSearchCV(SVC(kernel='linear'), param_distributions=param_dist, n_iter=10, cv=5,
    #                                        scoring='f1_micro')
    #
    # random_search_svm.fit(X_train, y_train)
    #
    # print("Najbolji parametri:", random_search_svm.best_params_)
    # print("Mikro F1-mera:", random_search_svm.best_score_)
    #
    # svm_classifier = SVC(kernel='linear', C=random_search_svm.best_params_['C'])
    # svm_classifier.fit(X_train, y_train)
    # y_pred_svm = svm_classifier.predict(X_test)
    #
    # f1_micro_svm = f1_score(y_test, y_pred_svm, average='micro')
    #
    # print("Mikro F1-mera koristeći SVM sa tačno nameštenim parametrom C:", f1_micro_svm)
    ...
    # lr_classifier = LogisticRegression(C=random_search_lr.best_params_['C'])
    #
    # lr_classifier.fit(X_train, y_train)
    # y_pred_lr = lr_classifier.predict(X_test)
    # f1_micro_lr = f1_score(y_test, y_pred_lr, average='micro')
    #
    # print("Mikro F1-mera koristeći Logističku regresiju sa tačno nameštenim parametrom C:", f1_micro_lr)
    ...
    random_param = randomizedSearch(X_train, y_train)
    nb_classifier = MultinomialNB(alpha=random_param)

    nb_classifier.fit(X_train, y_train)
    y_pred_nb = nb_classifier.predict(X_test)
    f1_micro_nb = f1_score(y_test, y_pred_nb, average='micro')

    # Ispisujemo rezultat
    print("Mikro F1-mera koristeći Naivni Bajes :     ", f1_micro_nb)
    ...
    # models = {
    #     "Logistička regresija": (LogisticRegression(), {'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000]}),
    #     "Naivni Bajes": (MultinomialNB(), {'alpha': [0.001, 0.01, 0.1, 1, 10, 100]}),
    #     "Mašine potpornih vektora": (SVC(), {'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000], 'kernel': ['linear', 'rbf']})
    # }

    # for name, (classifier, params) in models.items():
    #     print(f"Optimizacija modela: {name}")
    #     if name == "Logistička regresija":
    #         search = GridSearchCV(classifier, params, cv=5, scoring='f1_micro')
    #     else:
    #         search = RandomizedSearchCV(classifier, params, n_iter=5, cv=5, scoring='f1_micro')
